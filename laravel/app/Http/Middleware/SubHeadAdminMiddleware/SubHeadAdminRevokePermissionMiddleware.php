...
class SubHeadAdminRevokePermissionMiddleware
{
    /**
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        // Restrict revoking permissions from users with "SuperAdmin" or "HeadAdmin" roles
        if ($targetUser->hasRole(['SuperAdmin', 'HeadAdmin', 'SubHeadAdmin','HrHead'])) {
            return response()->json(['message' => 'You are not authorized to revoke permissions from users with these roles.'], 403);
        }

        // Ensure the SubAdmin can only revoke permissions they have
        ...
        if ($currentUser->branch_id !== $targetUser->branch_id) {
            return response()->json(['message' => 'You are not authorized to revoke permissions.'], 403);
        }
